...
def imp_match(img1, img2):
  hu1 = np.asarray(hu(img1)).reshape((7,1))
  hu2 = np.asarray(hu(img2)).reshape((7,1))

  for i in range(0,7):
    hu1[i] = -1* np.copysign(1.0, hu1[i]) * np.log10(abs(hu1[i]))
    hu2[i] = -1* np.copysign(1.0, hu2[i]) * np.log10(abs(hu2[i]))
  print(hu1)
  print(hu2)